...
int resolver_labirinto(int lab[MAX][MAX], int m, int n, int movX[], int movY[], int li, int ci, int lf, int cf){
	...
	*/
	...
	for (i = 0; i < 4; i++){
		...
		if ((l >= 0) && (l < m) && (c >= 0) && (c < n) && (lab[l][c] == -1)){ //-1 significa q aposicção n foi visitada
	...
	/*Todos os movimentos a partir da posição inicial são testados*/
		for (i = 0; i < 4; i++){
			...
			if ((l >= 0) && (l < m) && (c >= 0) && (c < n) && ((lab[l][c] == -1) || (lab[l][c] > lab[li][ci] + 1))){ //parte depois do ou permite passar pelo mesmo local 2 vezes
				lab[l][c] = lab[li][ci] + 1;                                        // ve se a nova é maior que a atual +1 se for é substituida por lab[li][ci]+1
