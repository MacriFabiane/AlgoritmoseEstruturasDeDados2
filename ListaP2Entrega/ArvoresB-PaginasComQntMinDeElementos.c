...
#include <math.h>
#include <stdlib.h>
#define N 5 // define a ordem como 5
...
int somaPgComMinElementos(NodeB *tree) { 
    int cont = 0;
    if (tree == NULL) {
        return 0;
    }
    if (tree->nro_chaves <= ceil((N/2))) { //analisar as q tem apenas dois elementos - min
        cont++;
    }

    for (int i = 0; i < tree->nro_chaves+1; i++) { // mais um pq a quant de filhos e igual a: elemento da pg +1
        cont += somaPgComMinElementos(tree->filhos[i]);
    }
...
int main() {
    ...
    cont=somaPgComMinElementos(tree);
    printf("%d", cont);
